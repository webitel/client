			...
			return this.dataListValue.map((item) => {
				...
				Object.keys(item.access).forEach((rule) => {
					access[rule] = {
						...item.access[rule],
						name: this.$t(
							`objects.permissions.object.accessMode.${item.access[rule].id}`,
						),
					};
				});
				return {
	...
	methods: {
		...mapActions({
			setParentId(dispatch, payload) {
				return dispatch(
					`${this.namespace}/${this.subNamespace}/SET_PARENT_ITEM_ID`,
					payload,
				);
			},
			loadDataList(dispatch, payload) {
				return dispatch(
					`${this.namespace}/${this.subNamespace}/LOAD_DATA_LIST`,
					payload,
				);
			},
			...
			setSearch(dispatch, payload) {
				return dispatch(
					`${this.namespace}/${this.subNamespace}/SET_SEARCH`,
					payload,
				);
			},
			nextPage(dispatch, payload) {
				return dispatch(
					`${this.namespace}/${this.subNamespace}/NEXT_PAGE`,
					payload,
				);
			},
			prevPage(dispatch, payload) {
				return dispatch(
					`${this.namespace}/${this.subNamespace}/PREV_PAGE`,
					payload,
				);
			},
			...
			changeReadAccessMode(dispatch, payload) {
				return dispatch(
					`${this.namespace}/${this.subNamespace}/CHANGE_READ_ACCESS_MODE`,
					payload,
				);
			},
			...
			resetState(dispatch, payload) {
				return dispatch(
					`${this.namespace}/${this.subNamespace}/RESET_ITEM_STATE`,
					payload,
				);
			},
		}),
