...
export function useWeekDaysData(namespace, dataName) {
  ...
  function setItemProp(payload) {
    return store.dispatch(`${namespace}/SET_CALENDAR_ITEM_PROPERTY`, { name: dataName, ...payload});
  }

  function addRange(day) {
    return store.dispatch(`${namespace}/ADD_RANGE`, { day, name: dataName });
  }

  function removeRange(index) {
    return store.dispatch(`${namespace}/REMOVE_RANGE`, { index, name: dataName});
  }
