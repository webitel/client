...
const isRingtoneSelected = computed(() => isCustomRingtone.value && ringtone.value.name);
...
const isRingtoneSaved = computed(() =>
  (!savedRingtone.value && !ringtone.value.name) || savedRingtone.value === ringtone.value.name
);
...
function restoreRingtone() {
  ...
  if (ringtoneName) {
    ...
    ringtone.value = options.value.find(item => item.name === ringtoneName) || {};
