...
const actions = {
	...
	UPDATE_ROLE_PERMISSION: (context, { index, permission }) => {
		const value = [
			...context.state.itemInstance.permissions,
		];
	...
	DELETE_BULK_PERMISSIONS: (context, deleted) => {
		const permissions = context.state.itemInstance.permissions.filter(
			({ id: permId }) => {
	...
	UPDATE_APPLICATION_SECTION_ACCESS: (
		context,
		{ app, section, value, custom = false },
	) => {
		const metadata = deepCopy(context.state.itemInstance.metadata);
...
const roles = new ObjectStoreModule({
	resettableState,
	headers,
})
