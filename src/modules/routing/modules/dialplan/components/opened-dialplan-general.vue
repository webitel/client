<template>
  <section>
    ...
    <div class="object-input-grid">
      ...
      <wt-select :clearable="false" :disabled="disableUserInput" :label="$t('objects.routing.schema', 1)"
        :search-method="loadFlows" :v="v.itemInstance.schema" :value="itemInstance.schema" required
        @input="setItemProp({ prop: 'schema', value: $event })" />
      <wt-input-text :disabled="disableUserInput" :label="$t('objects.routing.dialplan.pattern')" :v="v.itemInstance.pattern"
        :model-value="itemInstance.pattern" required @update:model-value="setItemProp({ prop: 'pattern', value: $event })" />
