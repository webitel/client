<template>
  ...
  >
    ...
    <template #main>
      <gateway-popup :shown="isGatewayPopup" @close="isGatewayPopup = false" />
        ...
        >
          <wt-table :data="dataList" :headers="headers" sortable @sort="sort">
            ...
            <template #state="{ item, index }">
              <wt-switcher :disabled="!hasEditAccess" :model-value="item.enable"
                @update:model-value="patchItem({ item, index, prop: 'enable', value: $event })" />
            </template>
            <template #status="{ item }">
              <wt-indicator :color="computeStatusClass(item.rState)" :text="computeStatusText(item.rState)" />
            </template>
