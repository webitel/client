<template>
  <section>
    ...
    <div class="object-input-grid">
      <wt-select :disabled="disableUserInput" :label="$t('objects.directory.users.defaultDevice')"
        :options="itemInstance.devices" :value="itemInstance.device" required track-by="id"
        @input="setItemProp({ prop: 'device', value: $event })" @reset="setItemProp({ prop: 'device', value: {} })" />
      <div>
        <wt-select :close-on-select="false" :disabled="disableUserInput"
          :label="$t('objects.directory.devices.devices', 2)" :search-method="loadDropdownOptionsList"
          :value="itemInstance.devices" multiple @input="setItemProp({ prop: 'devices', value: $event })" />
        <div class="hint-link__wrap typo-body-2">
          <span>{{ $t('objects.directory.users.deviceNotFound') }} </span>
          <adm-item-link id="new" class="hint-link__link typo-subtitle-2" :route-name="RouteNames.DEVICES">
