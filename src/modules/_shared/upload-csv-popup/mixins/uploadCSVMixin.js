...
export default {
	props: {
		...
		mappingFields: {
			type: Array,
			description:
				'[{ name: String, required: Boolean, csv: String, multiple: Boolean }]',
		},
		addBulkItems: {
			type: Function,
		},
	...
	computed: {
		...
		csvColumns() {
			return this.skipHeaders
				...
				: Object.keys(this.csvPreview[0]).map(
						(col, index) => `${index + 1} column`,
					);
		},
		parseCSVOptions() {
		...
		allowSaveAction() {
			return this.mappingFields.every(
	...
	methods: {
		...
		async processCSV() {
			...
			try {
				...
				const handler =
					this.handlingMode === HandlingCSVMode.PROCESS
