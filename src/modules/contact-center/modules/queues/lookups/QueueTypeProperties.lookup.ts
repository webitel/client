...
export interface QueueTypeProperty {
	/**
	 * i18n key for the type's display name. The create-queue popup reads its
	 * blurb from the same key suffixed with `Description`.
	 */
	locale: string;
	subpath: string;
	/** ids of the form controls this type shows — see useQueueTypeControls */
	controls: string[];
}

const QueueTypeProperties: Readonly<Record<number, QueueTypeProperty>> =
	Object.freeze({
